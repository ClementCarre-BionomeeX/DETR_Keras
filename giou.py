...
def _calculate_giou(b1, b2):
    ...
    return 1 - tf.squeeze(
        iou - tf.math.divide_no_nan((enclose_area - union_area), enclose_area)
    )
...
def box_giou(boxes1, boxes2):
    ...
    return tf.map_fn(
        curried_giou_per_batch,
        tf.range(tf.shape(boxes1)[0], dtype=tf.int64),
